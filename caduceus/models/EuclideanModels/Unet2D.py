...
class Unet2D:
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            # Inputs.
            M_0 = self._params["training_parameters"]["input_width"]["value"]* self._params["training_parameters"]["input_height"]["value"] 
            M_1 = self._params["training_parameters"]["output_width"]["value"]* self._params["training_parameters"]["output_height"]["value"] 
    ...
    def _inference(self, x, dropout, phase_train, reuse=False):
        x = tf.reshape(x, [self._params["training_parameters"]["batch_size"]["value"], self._params["training_parameters"]["input_width"]["value"], 
                        self._params["training_parameters"]["input_height"]["value"] ])
        ...
        for layer in range(layers):
            ...
            conv = self.conv2d(prev, features, conv_size, is_training=phase_train, scope='encoding' + str(layer))
            connection_outputs.append(conv)
            pool = tf.nn.max_pool(conv, ksize=[1, pool_kernel_size, pool_kernel_size, 1],
                                    strides=[1, pool_stride_size, pool_stride_size, 1],
                                    padding='SAME')
        
        bottom = self.conv2d(pool, 2**layers * self.features_root, conv_size, is_training=phase_train, scope='bottom')
        ...
        for layer in range(layers):
            ...
            deconv_output_shape = [tf.shape(prev)[0], shape[1] * deconv_size, shape[2] * deconv_size,
                                   features]
            deconv = self.deconv2d(prev, deconv_output_shape, deconv_size, is_training=phase_train,
                              scope='decoding' + str(conterpart_layer))
            # cc = self.crop_and_concat(connection_outputs[conterpart_layer], deconv)
            cc = tf.concat([connection_outputs[conterpart_layer], deconv], axis=3)
            conv_decoding = self.conv2d(cc, features, conv_size, is_training=phase_train,
                                   scope='decoding' + str(conterpart_layer))
            
        with tf.variable_scope('last_layer') as scope:
            # #x = self.conv3d(conv_decoding, 1, 1, is_training=phase_train,
            #                        scope='decoding_last')
            w1 = tf.get_variable('w1', [1, 1, conv_decoding.get_shape()[-1], 1],
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
            conv1 = tf.nn.conv2d(conv_decoding, w1, strides=[1, 1, 1, 1], padding='SAME')
    ...
    def conv2d(self, input_, output_dim, f_size, is_training, scope='conv3d'):
        with tf.variable_scope(scope) as scope:
            # VGG network uses two 3*3 conv layers to effectively increase receptive field
            w1 = tf.get_variable('w1', [f_size, f_size, input_.get_shape()[-1], output_dim],
                                initializer=tf.truncated_normal_initializer(stddev=0.1)) 
                                
            conv1 = tf.nn.conv2d(input_, w1, strides=[1, 1, 1, 1], padding='SAME') # NDHWC format
            ...
            w2 = tf.get_variable('w2', [f_size, f_size, output_dim, output_dim],
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
            conv2 = tf.nn.conv2d(r1, w2, strides=[1, 1, 1, 1], padding='SAME') # NDHWC format
    ...
    def deconv2d(self, input_, output_shape, f_size, is_training, scope='deconv3d'):
        with tf.variable_scope(scope) as scope:
            output_dim = output_shape[-1]
            w = tf.get_variable('w', [f_size, f_size, output_dim, input_.get_shape()[-1]],
                                initializer=tf.truncated_normal_initializer(stddev=0.1))
            deconv = tf.nn.conv2d_transpose(input_, w, output_shape, strides=[1, f_size, f_size, 1], padding='SAME')  # NDHWC format
