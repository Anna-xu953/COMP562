...
class Unet3D:
    ...
    def _inference(self, x, dropout, phase_train, reuse=False):
        ...
        for layer in range(layers):
            ...
            conv = self.conv3d(prev, features, conv_size, is_training=phase_train, scope='encoding' + str(layer))
        ...
        bottom = self.conv3d(pool, 2**layers * self.features_root, conv_size, is_training=phase_train, scope='bottom')
        ...
        for layer in range(layers):
            ...
            conv_decoding = self.conv3d(cc, features, conv_size, is_training=phase_train,
                                   scope='decoding' + str(conterpart_layer))
